...
const int lenToMidWall = 40;
const int lenToSideWall = 50;
...
std::array<2, int> get_min_pos() {
    int y = currentPos[0];
    int x = currentPos[1];

    int minPos[2] = {currentPos[0], currentPos[1]};

    if (!(walls[y][x] & WALL_UP) && in_bounds(x, y - 1)) { // если стены сверху нет
        if (maze[y-1][x] < maze[minPos[0]][minPos[1]]) {
            minPos[0] = y-1;
            minPos[1] = x;
        }
    }

    if (!(walls[y][x] & WALL_DOWN) && in_bounds(x, y + 1)) { // если стены снизу нет
        if (maze[y + 1][x] < maze[minPos[0]][minPos[1]]) {
            minPos[0] = y+1;
            minPos[1] = x;
        }
    ...
    if (!(walls[y][x] & WALL_RIGHT) && in_bounds(x+1, y)) { // если стены справа нет
        if (maze[y][x+1] < maze[minPos[0]][minPos[1]]) {
            minPos[0] = y;
            minPos[1] = x+1;
        }
    }

    if (!(walls[y][x] & WALL_UP) && in_bounds(x-1, y)) { // если стены слева нет
        if (maze[y][x-1] < maze[minPos[0]][minPos[1]]) {
            minPos[0] = y;
            minPos[1] = x-1;
        }
